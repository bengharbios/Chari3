...
function OtpVerification({ locale }: { locale: Locale }) {
  ...
  const submitOtp = async (code: string) => {
    ...
    try {
      ...
      // Timeout or network error — show error, don't silently fallback to registration
      clearTimeout(timeoutId);
      setIsVerifying(false);
      setError(t(locale, 'خطأ في الاتصال بالخادم. تأكد من اتصالك بالإنترنت وحاول مرة أخرى.', 'Server connection error. Check your internet and try again.'));
      setDigits(['', '', '', '', '', '']);
      focusInput(0);
    }
  };
