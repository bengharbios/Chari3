...
export function KycVerificationSection({ isAr }: { isAr: boolean }) {
  ...
  const fetchData = async () => {
    ...
    if (json.success) {
      ...
      if (json.merchantType) setMerchantType(json.merchantType);
    }
  };
  ...
  const requiredDocs = merchantType === 'business' 
    ? ['NATIONAL_ID_FRONT', 'NATIONAL_ID_BACK', 'BUSINESS_LICENSE']
    : ['NATIONAL_ID_FRONT', 'NATIONAL_ID_BACK']; // FREELANCE_DOCUMENT is optional typically

  const optionalDocs = merchantType === 'individual' ? ['FREELANCE_DOCUMENT'] : [];
  ...
  return (
    <Card className="mt-6 border-blue-200">
      ...
      <CardContent className="pt-6 space-y-4">
          ...
          return (
            <div key={docType} className="flex flex-col sm:flex-row items-center justify-between p-3 border rounded-lg gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="font-semibold text-sm">{docType.replace(/_/g, ' ')} <span className="text-red-500">*</span></h4>
                  {doc?.status === 'REJECTED' && (
        ...
        {optionalDocs.length > 0 && optionalDocs.map(docType => {
          const doc = docs.find((d: any) => d.type === docType);
          return (
            <div key={docType} className="flex flex-col sm:flex-row items-center justify-between p-3 border border-dashed rounded-lg gap-4">
              <div className="flex items-center gap-3">
                <FileText className="h-5 w-5 text-gray-400" />
                <div>
                  <h4 className="font-semibold text-sm">{docType.replace(/_/g, ' ')} <span className="text-gray-400 text-xs">({isAr ? 'اختياري' : 'Optional'})</span></h4>
                  {doc?.status === 'REJECTED' && (
                    <p className="text-xs text-red-600 mt-1">{isAr ? 'سبب الرفض:' : 'Reason:'} {doc.rejectionReason}</p>
                  )}
                </div>
        ...
        <div className="pt-4 border-t flex justify-end">
          <Button 
            ...
            disabled={docs.filter((d: any) => requiredDocs.includes(d.type)).length < requiredDocs.length || data.status === 'PENDING_REVIEW' || data.status === 'APPROVED'}
