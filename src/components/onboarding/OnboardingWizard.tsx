...
const IBAN_CONFIGS: Record<string, IbanConfig> = {
  DZ: { prefix: 'DZ', length: 24, placeholder: 'DZ0000000000000000000000', labelAr: 'رقم الحساب البنكي الدولي (IBAN جزائري / RIP)', labelEn: 'Algerian IBAN' },
...
const FINANCE_CONFIGS: Record<string, CountryFinanceConfig> = {
  DZ: {
    ...
    notes: [
      { ar: 'يمكنك تقديم كشف حساب CCP أو حساب بنكي عادية', en: 'You can provide a CCP or regular bank statement' },
      { ar: 'حساب BaridiMob مقبول كوسيلة دفع وتصفية حرة', en: 'BaridiMob account is accepted as an alternative payment method' },
    ],
  },
  SA: {
...
function getSteps(role: UserRole): StepDef[] {
  switch (role) {
    case 'store_manager':
      return [
        { titleAr: 'التوثيق القانوني والتراخيص', titleEn: 'Legal Verification', icon: <FileText className="h-4 w-4" /> },
        { titleAr: 'التوثيق المالي والحساب البنكي', titleEn: 'Financial Verification', icon: <Shield className="h-4 w-4" /> },
        { titleAr: 'توثيق الهوية الوطنية', titleEn: 'Identity Verification', icon: <User className="h-4 w-4" /> },
      ];
    case 'seller':
      return [
        { titleAr: 'إثبات وثيقة النشاط', titleEn: 'Eligibility Proof', icon: <FileText className="h-4 w-4" /> },
    ...
    async (file: File) => {
      ...
      try {
        ...
        if (err instanceof DOMException && err.name === 'AbortError') {
          ...
          toast.error(
            t(locale, 'خطأ في الاتصال. تحقق من الإنترنيت وحاول مرة أخرى.', 'Connection error. Check your internet and try again.')
          );
        }
      } finally {
    ...
    <div className="space-y-2">
      ...
      {showCamera && (
        ...
        >
          ...
          {t(locale, 'التقط صورة من الكاميرا', 'Take Photo')}
        </Button>
      )}
      <input
        ref={cameraRef}
...
export default function OnboardingWizard() {
  ...
  const canProceed = (): boolean => {
    switch (role) {
      ...
      case 'logistics':
        switch (currentStep) {
          case 0: return !!freelanceDocumentFile; // Driver License Front
          case 1: return !!transportLicenseFile; // Carte Grise
          case 2: return !!insuranceCertificateFile; // Insurance
          case 3: return true;
          default: return true;
        }
      default:
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (role === 'seller' && !freelancerIban) {
        ...
        missingMsg = t(locale, 'يرجى رفع رخصة القيادة والبطاقة الرمادية وتأمين المركبة', 'Please upload transport license and insurance');
      } else if (role === 'logistics' && (!numberOfVehicles || !numberOfDrivers)) {
        missingMsg = t(locale, 'يرجى تحديد معلومات المركبة والأسطول', 'Please enter number of vehicles and drivers');
      }
      ...
      try {
        ...
        if (data.success) {
          ...
          submitForReview();
          toast.success(t(locale, 'تم ارسال طلبك للمراجعة والاعتماد', 'Application submitted for review'));
        } else {
          const errorMsg = data.error
            || (data.missingFields
              ? t(locale, `╪¡┘é┘ê┘ä ┘à┘ü┘é┘ê╪»╪⌐: ${data.missingFields.join('╪î ')}`, `Missing fields: ${data.missingFields.join(', ')}`)
              : t(locale, 'فشل ارسال الطلب', 'Failed to submit application'));
          toast.error(errorMsg);
        }
      } catch (fetchErr: unknown) {
        if (fetchErr instanceof DOMException && fetchErr.name === 'AbortError') {
          toast.error(t(locale, 'انتهت مهلة الارسال. تحقق من اتصالك وحاول مجدداً.', 'Request timed out. Check your connection and try again.'));
        } else {
          console.error('[handleSubmit] Fetch error:', fetchErr);
          toast.error(t(locale, 'حدث خطأ أثناء ارسال الطلب. يرجى المحاولة مرة أخرى.', 'An error occurred while submitting. Please try again.'));
        }
      } finally {
        clearTimeout(timeout);
        setIsSubmitting(false);
            ...
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                <h3 className="font-semibold text-base">
                  {t(locale, 'التوثيق القانوني والتراخيص', 'Legal Verification')}
                </h3>
        ...
        return (
          <div className="space-y-5 animate-fade-in">
            ...
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                <h3 className="font-semibold text-base">
                  {t(locale, 'التوثيق المالي والحساب البنكي', 'Financial Verification')}
                </h3>
                <p className="text-xs text-muted-foreground">
          ...
          <div className="space-y-5 animate-fade-in">
            ...
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  {t(locale, 'الحساب البنكي / بريدي موب الشخصي', 'Personal bank account')}
                </p>
              </div>
            </div>
        ...
        return (
          <div className="space-y-5 animate-fade-in">
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                <h3 className="font-semibold text-base">
                  {t(locale, 'رخصة السياقة البيومترية (Permis de Conduire)', 'Biometric Driver License')}
                </h3>
                <p className="text-xs text-muted-foreground">
                  {t(locale, 'رخصة سياقة سارية المفعول مطابقة لصنف مركبة التوصيل (المادة 180 - المرسوم 04-381)', 'Valid driver license matching vehicle category (Executive Decree 04-381)')}
                </p>
              </div>
            </div>
            <FileUploadZone
              label="رخصة السياقة (الوجه الأمامي)"
              labelEn="Driver License (Front)"
              fileName={freelanceDocumentFile}
              onFileSelect={(name) => setField('freelanceDocumentFile', name)}
              onRemove={() => setField('freelanceDocumentFile', null)}
              locale={locale}
            ...
            />
            <FileUploadZone
              label="رخصة السياقة (الوجه الخلفي) / بطاقة الهوية"
              labelEn="Driver License (Back) / ID"
              fileName={idFrontFile}
              onFileSelect={(name) => setField('idFrontFile', name)}
              onRemove={() => setField('idFrontFile', null)}
              locale={locale}
        ...
        return (
          <div className="space-y-5 animate-fade-in">
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                <h3 className="font-semibold text-base">
                  {t(locale, 'البطاقة الرمادية للمركبة/الدراجة (Carte Grise)', 'Vehicle Registration (Carte Grise)')}
                </h3>
                <p className="text-xs text-muted-foreground">
                  {t(locale, 'وثيقة الملكية أو عقد استخدام المركبة المسجلة باسم السائق', 'Vehicle registration document under driver name')}
                </p>
              </div>
            </div>
            <FileUploadZone
              label="البطاقة الرمادية للمركبة (Carte Grise)"
              labelEn="Vehicle Registration Document"
      ...
      case 2:
        return (
          <div className="space-y-5 animate-fade-in">
            ...
            <FileUploadZone
              label="عقد تأمين المركبة والمسؤولية المدنية"
              labelEn="Vehicle Insurance Certificate"
      ...
      <div className="space-y-5 animate-fade-in">
        ...
        <div className="flex items-start gap-2 p-3 rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800">
          ...
          <p className="text-xs text-amber-700 dark:text-amber-300">
            {t(
              locale,
              'بعد إرسال المستندات، ستتم المراجعة والاعتماد خلال 2-24 ساعة عمل.',
          ...
          >
            {isSubmitting ? (
              <>
                ...
                {t(locale, 'جاري الإرسال والاعتماد...', 'Submitting...')}
              </>
            ) : (
              <>
                <Shield className="h-4 w-4 me-2" />
                {t(locale, 'إرسال للمراجعة والتوثيق', 'Submit for Review')}
              </>
            )}
          </Button>
        </div>
          ...
          <div className="mx-6 mt-5 flex items-center gap-2.5 p-3 rounded-lg bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800/30 animate-fade-in">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-blue-600 dark:text-blue-400 mt-0.5">
                {t(locale, 'يمكنك استكمال التوثيق من حيث توقفت', 'Continue from where you left off')}
              </p>
            </div>
          </div>
        )}
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-bold text-lg">
                {t(locale, 'معالج التوثيق والتحقق من الحساب', 'Complete Your Account')}
              </h2>
            </div>
            <Button
              variant="ghost"
            ...
            >
              ...
              {t(locale, 'حفظ كمسودة وخروج', 'Save Draft & Exit')}
            </Button>
          </div>
          {/* Progress bar */}
