...
export default function WalletPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (amt < minAmount) {
      toast.error(t(locale, `الحد الأدنى للسحب هو ${minAmount}`, `Minimum withdrawal is ${minAmount}`));
      return;
    }
    if (amt > availableBalance) {
      toast.error(t(locale, 'الرصيد المتاح غير كافٍ', 'Insufficient available balance'));
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        toast.success(t(locale, 'تم إرسال طلب السحب بنجاح قيد المراجعة', 'Withdrawal request submitted successfully'));
      ...
      toast.error(t(locale, 'حدث خطأ أثناء إرسال الطلب', 'Error submitting request'));
    } finally {
    ...
    <div dir={isRTL ? 'rtl' : 'ltr'} className="max-w-[1200px] mx-auto p-4 sm:p-6 lg:p-8 space-y-6 text-start font-cairo">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-black flex items-center gap-2 text-foreground">
            ...
            {t(locale, 'محفظتي وأرباحي', 'My Wallet & Earnings')}
          </h1>
          <p className="text-sm text-muted-foreground mt-1">
            {t(locale, 'اسحب أرباحك وتابع رصيدك المتاح وسجل السحوبات', 'Withdraw earnings and track your available balance')}
          </p>
        </div>
        <Button 
        ...
        >
          ...
          {t(locale, 'طلب سحب رصيد', 'Request Withdrawal')}
        </Button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="bg-gradient-to-br from-emerald-500 to-teal-600 text-white shadow-lg border-0">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium opacity-90">{t(locale, 'الرصيد الإجمالي', 'Total Balance')}</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card className="border-border bg-card">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">{t(locale, 'الرصيد المتاح للسحب', 'Available for Withdrawal')}</CardTitle>
          </CardHeader>
          <CardContent>
            ...
            {availableBalance < minAmount && (
              <p className="text-xs text-rose-500 mt-1 font-semibold">
                {t(locale, `الحد الأدنى للسحب هو ${minAmount} ${currency}`, `Minimum withdrawal is ${minAmount} ${currency}`)}
              </p>
            )}
          </CardContent>
        </Card>
        ...
        <Card className="border-border bg-card">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">{t(locale, 'طلبات قيد المعالجة', 'Pending Requests')}</CardTitle>
          </CardHeader>
          <CardContent>
      ...
      <Card className="border-border">
        <CardHeader>
          <CardTitle className="text-lg font-bold flex items-center gap-2">
            ...
            {t(locale, 'سجل السحوبات', 'Withdrawal History')}
          </CardTitle>
        </CardHeader>
        <CardContent>
            ...
            <Table>
              <TableHeader className="bg-muted/30">
                <TableRow>
                  ...
                  <TableHead className="text-start">{t(locale, 'وسيلة السحب', 'Method')}</TableHead>
                ...
                {withdrawals.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={5} className="text-center py-8 text-muted-foreground">
                      {t(locale, 'لا توجد طلبات سحب سابقة', 'No withdrawal history found')}
                    </TableCell>
                  </TableRow>
                ) : (
                  withdrawals.map(w => (
                    <TableRow key={w.id}>
                      ...
                      <TableCell className="text-center">
                        {w.status === 'pending' ? <Badge variant="outline" className="border-amber-500 text-amber-500 bg-amber-500/10"><Clock className="h-3 w-3 me-1"/> {t(locale, 'قيد المراجعة', 'Pending')}</Badge> :
                         w.status === 'paid' ? <Badge variant="outline" className="border-emerald-500 text-emerald-500 bg-emerald-500/10"><CheckCircle2 className="h-3 w-3 me-1"/> {t(locale, 'تم الدفع', 'Paid')}</Badge> :
                         w.status === 'rejected' ? <Badge variant="outline" className="border-rose-500 text-rose-500 bg-rose-500/10"><XCircle className="h-3 w-3 me-1"/> {t(locale, 'مرفوض', 'Rejected')}</Badge> :
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent dir={isRTL ? 'rtl' : 'ltr'} className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle className="text-start">{t(locale, 'طلب سحب رصيد', 'Request Withdrawal')}</DialogTitle>
            <DialogDescription className="text-start">
              {t(locale, 'سيتم مراجعة طلبك من قبل الإدارة وإرسال المبلغ في أقرب وقت ممكن.', 'Your request will be reviewed by admin and transferred soon.')}
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={handleSubmit} className="space-y-4 pt-4">
            <div className="space-y-2">
              <Label className="text-start block">{t(locale, 'المبلغ المطلوب (د.ج)', 'Amount (DZD)')}</Label>
              <Input
                type="number"
                min={minAmount}
              ...
              <p className="text-[10px] text-muted-foreground text-start">
                {t(locale, `المتاح: ${availableBalance} دج`, `Available: ${availableBalance} DZD`)}
              </p>
            </div>
            <div className="space-y-2 text-start block">
              <Label>{t(locale, 'طريقة السحب', 'Withdrawal Method')}</Label>
              <Select value={formMethod} onValueChange={setFormMethod} required>
                <SelectTrigger className="text-start w-full">
                  <SelectValue placeholder={t(locale, 'اختر الطريقة', 'Select Method')} />
                </SelectTrigger>
                <SelectContent>
                  ...
                  {methods.includes('bank_transfer') && <SelectItem value="bank_transfer">{t(locale, 'تحويل بنكي', 'Bank Transfer')}</SelectItem>}
                  {methods.includes('cash') && <SelectItem value="cash">{t(locale, 'استلام نقدي', 'Cash Pickup')}</SelectItem>}
                </SelectContent>
              </Select>
            </div>
            ...
            {formMethod && (
              <>
                {formMethod === 'bank_transfer' && (
                  <div className="space-y-2">
                    <Label className="text-start block">{t(locale, 'اسم البنك', 'Bank Name')}</Label>
                    <Input required value={formBankName} onChange={e => setFormBankName(e.target.value)} />
                  </div>
                )}
                <div className="space-y-2">
                  <Label className="text-start block">{t(locale, 'اسم صاحب الحساب', 'Account Holder Name')}</Label>
                  <Input required value={formAccountName} onChange={e => setFormAccountName(e.target.value)} />
                </div>
                <div className="space-y-2">
                  <Label className="text-start block">{t(locale, 'رقم الحساب (RIP/RIB)', 'Account Number')}</Label>
            ...
            <DialogFooter className="pt-4 flex sm:justify-start">
              ...
              <Button type="submit" disabled={isSubmitting || !formMethod} className="bg-emerald-500 hover:bg-emerald-600">
                ...
                {t(locale, 'تأكيد الطلب', 'Confirm Request')}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
