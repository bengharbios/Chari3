...
import { ShieldCheck, FileCheck, CheckCircle2, Clock, Upload, AlertCircle } from 'lucide-react';
  ...
  const docs = [
    {
      ...
      descAr: 'رخصة سياقة سارية المفعول مطابقة لصنف المركبة (أ1، أ2، ب، ج1، ج2).',
      descEn: 'Valid driving license matching vehicle category.',
    ...
    },
    {
      ...
      descAr: 'وثيقة الملكية أو عقد استخدام المركبة المسجلة باسم المندوب أو الشركة.',
      descEn: 'Vehicle registration document under driver/company name.',
      status: isVerified ? 'verified' : 'pending',
    },
    {
      id: 'insurance',
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-2xl font-black">{t('وثائق التوثيق ورخصة المندوب (KYC)', 'Driver Documents & Verification')}</h1>
              <Badge className="bg-emerald-500/10 text-emerald-500 border-emerald-500/20 font-bold">
                <ShieldCheck className="h-3.5 w-3.5 me-1" />
                {isVerified ? t('حساب موثق رسمياً', 'Officially Verified') : t('قيد مراجعة الإدارة', 'Under Review')}
              </Badge>
            </div>
